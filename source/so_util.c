...
// ---------------------------------------------------------------------------
// Trampoline pool: lets a hook run the ORIGINAL function after inspecting args.
// hook_arm64 overwrites the target's first 16 bytes (4 instrs) with a jump, so
// the original prologue is lost. A trampoline holds [relocated prologue][jump to
// target+16], callable as the original. Prologues here are position-independent
// (sp-relative stp/sub), so copying them is safe. Backed by RW memory aliased as
// executable code (svcMapProcessCodeMemory), set RX before use.
// ---------------------------------------------------------------------------
static u8 *g_tramp_rw, *g_tramp_rx;
static size_t g_tramp_used, g_tramp_cap;
static VirtmemReservation *g_tramp_memrv;

// IMPORTANT: svcMapProcessCodeMemory makes the SOURCE buffer inaccessible (only
// the code alias remains). So we must WRITE all trampolines to the RW backing
// FIRST, and only map+RX it in finalize (mirrors so_load: relocate base, then
// so_finalize maps). init just reserves the buffer + virtual code range.
void so_tramp_init(size_t cap) {
  g_tramp_cap = ALIGN_MEM(cap, 0x1000);
  g_tramp_rw = memalign(0x1000, g_tramp_cap);
  if (!g_tramp_rw) fatal_error("tramp: OOM");
  memset(g_tramp_rw, 0, g_tramp_cap);
  virtmemLock(); // REQUIRED: find+reserve must hold the lock, else the range can
  g_tramp_rx = virtmemFindCodeMemory(g_tramp_cap, 0x1000); // clash with the game's
  g_tramp_memrv = virtmemAddReservation(g_tramp_rx, g_tramp_cap); // code mapping
  virtmemUnlock();
  g_tramp_used = 0;
}

// prologue_src: readable copy of the target's first 16 bytes (e.g. load_base+off).
// target_rx_plus16: final RX address of target+16 (e.g. load_virtbase+off+16).
uintptr_t so_make_tramp(const void *prologue_src, uintptr_t target_rx_plus16) {
  if (g_tramp_used + 32 > g_tramp_cap) fatal_error("tramp: full");
  u8 *rw = g_tramp_rw + g_tramp_used;
  memcpy(rw, prologue_src, 16);
  uint32_t *j = (uint32_t *)(rw + 16);
  j[0] = 0x58000051u; // LDR X17, #8
  j[1] = 0xd61f0220u; // BR X17
  *(uint64_t *)(j + 2) = target_rx_plus16;
  uintptr_t ret = (uintptr_t)(g_tramp_rx + g_tramp_used); // executable alias addr
  g_tramp_used += 32;
  return ret;
}

void so_tramp_finalize(void) {
  if (!g_tramp_rx) return;
  armDCacheFlush(g_tramp_rw, g_tramp_cap); // flush writes to RAM while still accessible
  Result rc = svcMapProcessCodeMemory(envGetOwnProcessHandle(), (u64)g_tramp_rx, (u64)g_tramp_rw, g_tramp_cap);
  if (R_FAILED(rc)) fatal_error("tramp: MapProcessCodeMemory %08x", rc);
  rc = svcSetProcessMemoryPermission(envGetOwnProcessHandle(), (u64)g_tramp_rx, g_tramp_cap, Perm_Rx);
  if (R_FAILED(rc)) fatal_error("tramp: SetPerm %08x", rc);
  armICacheInvalidate(g_tramp_rx, g_tramp_cap);
}
