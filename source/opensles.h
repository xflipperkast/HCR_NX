 ...
 * the Play and Volume interfaces) and back the buffer-queue players with the
 * Switch audio output service, using one software mixer.
 */
...
// Cocos2d-x SimpleAudioEngine bridge (Android normally implements these in
// Cocos2dxActivity; on Switch they are mixed by opensles.c).
void cocos_audio_preload(const char *path);
...
void cocos_audio_set_effects_volume(float volume);
float cocos_audio_effects_volume(void);
...
void cocos_audio_set_background_volume(float volume);
float cocos_audio_background_volume(void);
