...
#define INSTALL_ARCHIVE DATA_DIR "/" GAME_APK
...
static void progress_update(const char *name, uint64_t done, uint64_t total) {
  ...
  consoleClear();
  printf("Hill Climb Racing first-run installer\n\n");
  printf("Extracting: %s\n\n[", g_progress_name);
...
static void progress_finish(void) {
  if (!g_console_active) return;
  consoleClear();
  printf("Installation complete. Starting Hill Climb Racing...\n");
  consoleUpdate(NULL);
...
static int required_files_present(void) {
  char path[PATH_MAX];
  const char *const required[] = {SO_NAME, GAME_APK};
...
int installer_prepare_game_files(void) {
  ...
  if (!file_exists_with_data(INSTALL_ARCHIVE)) {
    set_error("Missing game files. Copy hcr.apk next to hcr_nx.nro and relaunch.");
    return -1;
  }
  progress_begin();
  progress_update("Opening hcr.apk", 0, 1);
  const int result = install_apk(INSTALL_ARCHIVE);
  if (result < 0 || !required_files_present()) {
    if (!g_error[0]) set_error("The APK did not contain Hill Climb Racing ARM64 files.");
    progress_abort();
    return -1;
  }
  progress_finish();
